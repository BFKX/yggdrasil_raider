...
public class Map {
	...
	public void display(GraphicsContext gc, @NotNull Coordinate characterPosition) {
		double initColumn = characterPosition.getX() - 90;
		double initLine = characterPosition.getY() - 60;
		double lineOffset = (characterPosition.getY()) - HEIGHT/(2* SIDE);
		double columnOffset =  (characterPosition.getX()) - WIDTH/(2*SIDE);
		for (double column = initColumn; column < initColumn + 180; column++) {
			for (double line = initLine; line < initLine + 120; line++) {
				if (line < 0 || line >= lines || column < 0 || column >= columns) {
					continue;
				}
				gc.drawImage(spriteSelector(map[(int)column][(int)line]),
